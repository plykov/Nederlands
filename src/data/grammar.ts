...
export const GRAMMAR_NOTES: GrammarNote[] = [
  ...
  },
  {
    ...
    gist: "Пустое подлежащее, «там», «об этом» — всё это одно короткое er.",
    pairs: [
    ...
    body:
      "У er пять разных работ, и учить их лучше по очереди, а не разом. Первая и самая частая: пустое подлежащее при неопределённом предмете — «er is een probleem», «er komt iemand». Вторая: «там» — «ik ben er geweest». Третья: замена неодушевлённого после предлога, причём предлог приклеивается — erover, ermee, ervoor — и часто разрывается надвое: «daar weet ik niets van». Четвёртая: счётное «их» — «ik heb er drie». В русском ни одной из этих функций отдельного слова не соответствует, поэтому er систематически теряется — а без него фраза звучит оборванной.",
  },
  {
    id: "hebben-zijn",
